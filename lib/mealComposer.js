...
export function composeSlot({ mealTime, targetCal, dishes, extras, recentIds, dayDatesUsed, seed }) {
  ...
  // extra_types where picking 2+ different items in one slot is unrealistic
  const SINGLE_PER_SLOT_TYPES = new Set(['juice', 'dairy', 'nuts', 'dates'])

  function addExtra(pool, remainCal) {
    const candidates = pool
      .filter(e => !items.some(i => i.recipe.id === e.id))
      .filter(e => {
        // At most 1 item per juice/dairy/nuts type in a slot
        if (SINGLE_PER_SLOT_TYPES.has(e.extra_type) &&
            items.some(i => i.recipe.extra_type === e.extra_type)) return false
