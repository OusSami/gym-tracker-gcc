      ...
      <div style={{maxWidth:520,margin:'0 auto',padding:'0 16px'}}>
          ...
          <div style={{paddingTop:14,paddingBottom:100}}>
            {mealPlanLoading ? (
              <div style={{textAlign:'center',padding:'60px 0',color:'var(--text-secondary)'}}>
                <div style={{width:28,height:28,border:'3px solid rgba(0,0,0,0.10)',borderTopColor:'var(--text-primary)',borderRadius:'50%',animation:'spin .8s linear infinite',margin:'0 auto 14px'}}/>
              ...
              <div style={{textAlign:'center',padding:'60px 0',color:'var(--text-secondary)'}}>
                ...
                <div style={{fontSize:'.78rem',color:'var(--text-secondary)'}}>ابدأ برنامجك لتفعيل خطة التغذية</div>
              </div>
            )}
          </div>
        )}
          ...
          <div style={{paddingTop:14}}>
            ...
            <div style={{background:'rgba(6,182,212,0.06)',border:'1px solid rgba(6,182,212,0.18)',borderRadius:14,padding:'13px 16px',marginBottom:12}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                ...
                <div style={{flex:1,margin:'0 12px'}}>
                  ...
                  <div style={{fontSize:'.62rem',color:'var(--text-secondary)',marginTop:3,textAlign:'right'}}>{pct(totalWater,G.water_ml||2500)}%</div>
                </div>
              </div>
                          ...
                          <div key={m.id} style={{borderBottom:'1px solid rgba(0,0,0,0.08)',paddingBottom:10,marginBottom:10}}>
                            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:4}}>
                              <div style={{flex:1}}>
                                ...
                                {m.portion_note&&<div style={{fontSize:'.68rem',color:'var(--text-secondary)',marginTop:1}}>{m.portion_note}</div>}
                              </div>
                              <div style={{display:'flex',alignItems:'center',gap:6}}>
                                <span style={{fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,color:'var(--text-primary)',fontSize:'.95rem'}}>{m.total_calories}</span>
                                <button onClick={()=>setEditMeal(m)} style={{background:'rgba(0,0,0,0.06)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:6,color:'var(--text-secondary)',cursor:'pointer',fontSize:'.7rem',padding:'3px 7px',fontFamily:"'DM Sans','Tajawal',sans-serif"}}>🔄</button>
            ...
            {meals.length===0&&(
              <div style={{textAlign:'center',padding:'30px 0',color:'var(--text-secondary)'}}>
                ...
                <button onClick={()=>{setTab('add');resetAdd()}} style={{background:'#111111',border:'none',borderRadius:12,padding:'12px 22px',fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,fontSize:'.9rem',color:'#FFFFFF',cursor:'pointer',marginTop:10}}>سجّل أول وجبة</button>
              </div>
            )}
          </div>
        )}
        ...
        {tab==='add' && (
          <div style={{paddingTop:16}}>
            {addStep==='type' && (
              <div>
                <div style={{fontSize:'.62rem',fontWeight:700,letterSpacing:1.5,color:'var(--text-secondary)',marginBottom:14}}>نوع الوجبة</div>
            ...
            {addStep==='capture' && (
              <div>
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14}}>
                  <button onClick={()=>setAddStep('type')} style={{background:'none',border:'none',color:'var(--text-secondary)',cursor:'pointer',fontSize:'1.1rem'}}>←</button>
                ...
                <div onClick={()=>{fileRef.current.removeAttribute('capture');fileRef.current.click()}}
                  style={{border:`2px dashed ${imgPreview?'rgba(0,0,0,0.08)':'rgba(0,0,0,0.12)'}`,borderRadius:16,overflow:'hidden',marginBottom:10,cursor:'pointer',minHeight:imgPreview?0:130,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.05)'}}>
                  ...
                  :<div style={{textAlign:'center',padding:'24px'}}><div style={{fontSize:'2.5rem',marginBottom:6}}>📸</div><div style={{color:'var(--text-secondary)',fontSize:'.85rem',fontWeight:600}}>صوّر وجبتك 📸</div></div>}
                </div>
                <input ref={fileRef} type="file" accept="image/*" style={{display:'none'}} onChange={e=>loadImg(e.target.files[0])}/>
                <div style={{display:'flex',gap:8,marginBottom:10}}>
                  <button onClick={()=>{fileRef.current.removeAttribute('capture');fileRef.current.click()}} style={{flex:1,padding:'10px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:10,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontSize:'.82rem',fontWeight:600}}>🖼 Gallery</button>
                  <button onClick={()=>{fileRef.current.setAttribute('capture','environment');fileRef.current.click()}} style={{flex:1,padding:'10px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:10,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontSize:'.82rem',fontWeight:600}}>📷 Camera</button>
                    ...
                    <div style={{marginTop:8}}>
                      <div style={{position:'relative',marginBottom:8}}>
                        <input
                          ...
                          style={{width:'100%',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.10)',color:'var(--text-primary)',padding:'10px 14px',borderRadius:10,outline:'none',fontFamily:"'Tajawal',sans-serif",fontSize:'.88rem',direction:'rtl',boxSizing:'border-box'}}
                          autoFocus
                        />
                        {gccLoading&&<div style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',width:14,height:14,border:'2px solid rgba(0,0,0,0.10)',borderTopColor:'#111111',borderRadius:'50%',animation:'spin .7s linear infinite'}}/>}
                      </div>
                          ...
                          {gccResults.map(food=>(
                            <div key={food.id} onClick={()=>useGccFood(food)}
                              ...
                              <div style={{textAlign:'left',flexShrink:0,paddingRight:4}}>
                                <div style={{fontWeight:900,color:'var(--text-primary)',fontFamily:'monospace',fontSize:'.92rem'}}>{food.calories}</div>
                                <div style={{fontSize:'.62rem',color:'var(--text-secondary)'}}>سعرة</div>
                              </div>
                              <div style={{display:'flex',gap:6,marginRight:8,flexShrink:0}}>
                                {[['B',food.protein_g,'#3b82f6'],['C',food.carbs_g,'#f97316'],['F',food.fat_g,'#22c55e']].map(([l,v,c])=>(
                                  <div key={l} style={{textAlign:'center'}}>
                                    ...
                                    <div style={{fontSize:'.55rem',color:'var(--text-secondary)'}}>{l}</div>
                                  </div>
                                ))}
                              </div>
                            </div>
                      ...
                      {gccSearch.length>=2&&!gccLoading&&gccResults.length===0&&(
                        <div style={{textAlign:'center',padding:'14px',color:'var(--text-secondary)',fontSize:'.82rem',fontFamily:"'Tajawal',sans-serif"}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                  <div style={{flex:1,height:1,background:'rgba(0,0,0,0.08)'}}/><span style={{color:'var(--text-secondary)',fontSize:'.68rem',fontWeight:700,whiteSpace:'nowrap'}}>أو حلّل بالذكاء</span><div style={{flex:1,height:1,background:'rgba(0,0,0,0.08)'}}/>
                </div>
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                  <div style={{flex:1,height:1,background:'rgba(0,0,0,0.08)'}}/><span style={{color:'var(--text-secondary)',fontSize:'.68rem',fontWeight:700,whiteSpace:'nowrap'}}>أو صفها بكلامك</span><div style={{flex:1,height:1,background:'rgba(0,0,0,0.08)'}}/>
                </div>
                ...
                {textInput&&imgB64&&<div style={{color:'var(--text-secondary)',fontSize:'.7rem',marginBottom:8,fontWeight:600}}>✓ AI cross-checks photo + description</div>}

                {/* Barcode scanner */}
                <div style={{display:'flex',alignItems:'center',gap:8,margin:'4px 0 8px'}}>
                  <div style={{flex:1,height:1,background:'rgba(0,0,0,0.08)'}}/><span style={{color:'var(--text-secondary)',fontSize:'.68rem',fontWeight:700,whiteSpace:'nowrap'}}>أو امسح الباركود</span><div style={{flex:1,height:1,background:'rgba(0,0,0,0.08)'}}/>
                </div>
                <div style={{display:'flex',gap:8,marginBottom:10}}>
                  ...
                  <button onClick={lookupBarcode} disabled={!barcode.trim()||barcodeLoading}
                    style={{padding:'0 13px',background:barcode.trim()?'rgba(6,182,212,0.12)':'rgba(0,0,0,0.04)',border:'1px solid '+(barcode.trim()?'rgba(6,182,212,0.35)':'rgba(0,0,0,0.08)'),borderRadius:10,color:barcode.trim()?'#06b6d4':'var(--text-secondary)',cursor:barcode.trim()?'pointer':'not-allowed',fontFamily:"'DM Sans','Tajawal',sans-serif",fontWeight:700,fontSize:'.8rem',height:44,display:'flex',alignItems:'center'}}>
                    {barcodeLoading?'…':'🔍'}
                  </button>
                </div>
                ...
                {customMeals.length>0&&(
                  <div style={{marginBottom:10}}>
                    <button onClick={()=>setShowCustom(v=>!v)} style={{width:'100%',padding:'10px 14px',background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:10,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontWeight:600,fontSize:'.82rem',textAlign:'right',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                      <span>⭐ My Saved Meals ({customMeals.length})</span>
                      <span style={{color:'var(--text-secondary)'}}>{showCustom?'▲':'▼'}</span>
                    </button>
                    {showCustom&&(
                        ...
                        {customMeals.map(m=>(
                          <button key={m.id} onClick={()=>useCustomMeal(m)}
                            ...
                            <div>
                              ...
                              <div style={{fontSize:'.7rem',color:'var(--text-secondary)',marginTop:2}}>{m.portion_note||''} · used {m.times_used}x</div>
                            </div>
                            <span style={{fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,color:'var(--text-primary)',fontSize:'.9rem'}}>{m.total_calories} kcal</span>
                          </button>
                        ))}
                      </div>
                    )}
                ...
                {mealType && recentMealsByType[mealType]?.length > 0 && (
                  <div style={{marginBottom:10}}>
                    <button onClick={()=>setShowRecentMeals(v=>!v)}
                      ...
                      <span style={{color:'var(--text-secondary)'}}>{showRecentMeals?'▲':'▼'}</span>
                    </button>
                    {showRecentMeals && (
                      <div style={{marginTop:6,display:'flex',flexDirection:'column',gap:5}}>
                        {recentMealsByType[mealType].map((m,i) => (
                          <button key={i} onClick={async()=>{
                            ...
                            <div>
                              ...
                              <div style={{fontSize:'.7rem',color:'var(--text-secondary)',marginTop:2}}>{m.portion_note||''}</div>
                            </div>
                            <span style={{fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,color:'var(--text-primary)',fontSize:'.9rem'}}>{m.total_calories} kcal</span>
                          </button>
                        ))}
                        <button onClick={async()=>{
                          ...
                          style={{padding:'9px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.10)',borderRadius:10,color:'var(--text-primary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontWeight:700,fontSize:'.8rem'}}>
                ...
                {analyzing?<div style={{textAlign:'center',padding:'24px'}}><div style={{width:40,height:40,border:'3px solid rgba(0,0,0,0.10)',borderTopColor:'#111111',borderRadius:'50%',animation:'spin .8s linear infinite',margin:'0 auto 12px'}}/><div style={{color:'var(--text-primary)',fontWeight:700,fontFamily:"'Space Grotesk','Tajawal',sans-serif",letterSpacing:1}}>جاري التحليل...</div></div>
                :<button onClick={analyze} disabled={!imgB64&&!textInput.trim()} style={{width:'100%',padding:'15px',background:(!imgB64&&!textInput.trim())?'rgba(0,0,0,0.05)':'#111111',border:'none',borderRadius:12,fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,fontSize:'.95rem',color:(!imgB64&&!textInput.trim())?'rgba(0,0,0,0.20)':'#FFFFFF',cursor:(!imgB64&&!textInput.trim())?'not-allowed':'pointer'}}>تحليل الوجبة</button>}
              </div>
            )}
          ...
          <div style={{paddingTop:14}}>
            ...
            <div style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.10)',borderRadius:14,padding:'14px 16px',marginBottom:14,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div>
                ...
                <div style={{fontSize:'.72rem',color:'var(--text-secondary)',marginTop:2}}>هدفك: {G.calories} سعرة · {pct(totals.calories||0,G.calories)}%</div>
              </div>
              {!goals&&<div style={{fontSize:'.72rem',color:'var(--text-secondary)',maxWidth:120,textAlign:'right',lineHeight:1.4}}>أكمل بياناتك للأهداف الشخصية</div>}
            </div>
                ...
                <div key={group} style={{marginBottom:14}}>
                  ...
                  <div style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:14,padding:'10px 14px'}}>
                    {groupNutrients.map((n, i) => {
                      ...
                      return (
                        <div key={n.key} style={{padding:'8px 0',borderBottom:i<groupNutrients.length-1?'1px solid rgba(0,0,0,0.06)':'none'}}>
                          <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}>
                            <span style={{fontSize:'.82rem',color:'var(--text-secondary)',fontWeight:500}}>{n.label}</span>
                            <div style={{display:'flex',alignItems:'baseline',gap:5}}>
                              ...
                              {goalVal>0&&<span style={{fontSize:'.68rem',color:over?'#ef4444':'rgba(0,0,0,0.30)',fontWeight:over?700:400}}>/ {fmt(goalVal)}{n.unit}</span>}
                              {goalVal>0&&<span style={{fontSize:'.65rem',color:over?'#ef4444':p>=80?'#4ade80':'rgba(0,0,0,0.35)',fontWeight:700,minWidth:32,textAlign:'right'}}>{p}%{over&&' ⚠'}</span>}
                            </div>
                          </div>
                          {goalVal>0&&(
            ...
            {!goals&&<div style={{background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:12,padding:'12px 16px',fontSize:'.8rem',color:'var(--text-secondary)',lineHeight:1.6}}>
          ...
          <div style={{paddingTop:16}}>
            ...
            {!report && !reportLoading && (
              <div style={{textAlign:'center',padding:'20px 0'}}>
                ...
                <div style={{color:'var(--text-secondary)',fontSize:'.82rem',marginBottom:20,lineHeight:1.6}}>
                ...
                {!meals.length
                  ? <div style={{color:'var(--text-secondary)',fontSize:'.82rem'}}>Log some meals first to generate a report.</div>
                  : <button onClick={generateReport}
                      style={{background:'#111111',border:'none',borderRadius:14,padding:'15px 28px',fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,fontSize:'1rem',color:'#FFFFFF',cursor:'pointer',boxShadow:'0 4px 12px rgba(0,0,0,0.12)'}}>
                      Analyze My Day ←
                    </button>
                }
            ...
            {reportLoading && (
              <div style={{textAlign:'center',padding:'40px 0'}}>
                <div style={{width:48,height:48,border:'3px solid rgba(0,0,0,0.10)',borderTopColor:'#111111',borderRadius:'50%',animation:'spin .8s linear infinite',margin:'0 auto 16px'}}/>
                <div style={{fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:700,color:'var(--text-primary)',letterSpacing:1}}>ANALYZING YOUR NUTRITION...</div>
                <div style={{color:'var(--text-secondary)',fontSize:'.78rem',marginTop:6}}>Reviewing {meals.length} meal{meals.length>1?'s':''} logged today</div>
              </div>
            )}
              ...
              <div>
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:reportIsStale?8:16}}>
                  ...
                  <button onClick={generateReport} disabled={reportLoading}
                    style={{background:'rgba(0,0,0,0.05)',border:'1px solid rgba(0,0,0,0.10)',borderRadius:9,padding:'6px 13px',color:'var(--text-primary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontSize:'.75rem',fontWeight:700,opacity:reportLoading?.6:1}}>
                ...
                {reportIsStale && (
                  <div style={{background:'rgba(234,179,8,0.08)',border:'1px solid rgba(234,179,8,0.25)',borderRadius:11,padding:'10px 14px',marginBottom:14,display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}}>
                    <div>
                      ...
                      <div style={{fontSize:'.7rem',color:'var(--text-secondary)'}}>This report was generated before your latest meal edits.</div>
                    </div>
                      ...
                      {(() => {
                        ...
                        return (
                          <svg width={88} height={88} viewBox="0 0 88 88">
                            ...
                            <text x={44} y={54} textAnchor="middle" fill="rgba(0,0,0,0.35)" fontSize={9} fontFamily="'DM Sans','Tajawal',sans-serif">/10</text>
                          </svg>
                        )
                      })()}
                    </div>
                    <div style={{flex:1}}>
                      <div style={{fontSize:'.62rem',fontWeight:700,letterSpacing:1.5,color:'var(--text-secondary)',marginBottom:5}}>تقييم يومك</div>
                      <div style={{fontSize:'.85rem',color:'var(--text-primary)',lineHeight:1.55}}>{report.summary}</div>
                    </div>
                  </div>
                  {/* Macro status pills */}
                ...
                {report.meal_reports && (
                  <div style={{marginBottom:14}}>
                    <div style={{fontSize:'.62rem',fontWeight:700,letterSpacing:1.5,color:'var(--text-secondary)',marginBottom:10}}>MEAL-BY-MEAL BREAKDOWN</div>
                        ...
                        <div key={mt} style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:14,padding:'13px 14px',marginBottom:8}}>
                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                            <div style={{display:'flex',alignItems:'center',gap:8}}>
                              ...
                              {!mealData.length && <span style={{fontSize:'.68rem',color:'var(--text-secondary)'}}>not logged</span>}
                            </div>
                          ...
                          {mr && (
                            <>
                              {mr.assessment && <div style={{fontSize:'.8rem',color:'var(--text-secondary)',lineHeight:1.5,marginBottom:8}}>{mr.assessment}</div>}
                              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                                {mr.positives?.length > 0 && (
                                  <div style={{background:'rgba(74,222,128,0.04)',border:'1px solid rgba(74,222,128,0.12)',borderRadius:9,padding:'8px 10px'}}>
                                    ...
                                    {mr.positives.map((p,i) => <div key={i} style={{fontSize:'.75rem',color:'var(--text-secondary)',marginBottom:3,lineHeight:1.4}}>+ {p}</div>)}
                                  </div>
                                )}
                                {mr.improvements?.length > 0 && (
                                  <div style={{background:'rgba(248,113,113,0.04)',border:'1px solid rgba(248,113,113,0.12)',borderRadius:9,padding:'8px 10px'}}>
                                    ...
                                    {mr.improvements.map((p,i) => <div key={i} style={{fontSize:'.75rem',color:'var(--text-secondary)',marginBottom:3,lineHeight:1.4}}>! {p}</div>)}
                                  </div>
                                )}
                              </div>
                            </>
                  ...
                  <div style={{background:'rgba(74,222,128,0.05)',border:'1px solid rgba(74,222,128,0.15)',borderRadius:14,padding:'13px 14px',marginBottom:10}}>
                    ...
                    {report.what_went_well.map((w,i) => (
                      <div key={i} style={{display:'flex',gap:8,marginBottom:7}}>
                        ...
                        <span style={{fontSize:'.82rem',color:'var(--text-secondary)',lineHeight:1.5}}>{w}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {report.nutrients_of_concern?.length > 0 && (
                  <div style={{background:'rgba(234,179,8,0.05)',border:'1px solid rgba(234,179,8,0.18)',borderRadius:14,padding:'13px 14px',marginBottom:10}}>
                    ...
                    {report.nutrients_of_concern.map((n,i) => (
                      <div key={i} style={{fontSize:'.82rem',color:'var(--text-secondary)',marginBottom:4}}>⚠ {n}</div>
                    ))}
                  </div>
                )}
                ...
                {report.meal_timing_note && (
                  <div style={{background:'rgba(129,140,248,0.05)',border:'1px solid rgba(129,140,248,0.15)',borderRadius:14,padding:'12px 14px',marginBottom:10}}>
                    ...
                    <div style={{fontSize:'.82rem',color:'var(--text-secondary)',lineHeight:1.5}}>{report.meal_timing_note}</div>
                  </div>
                )}
                ...
                {report.tomorrow_tips?.length > 0 && (
                  <div style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:14,padding:'13px 14px',marginBottom:16}}>
                    ...
                    {report.tomorrow_tips.map((t,i) => (
                      <div key={i} style={{display:'flex',gap:8,marginBottom:7}}>
                        <span style={{color:'var(--text-primary)',flexShrink:0,fontSize:'.85rem'}}>💡</span>
                        <span style={{fontSize:'.82rem',color:'var(--text-secondary)',lineHeight:1.5}}>{t}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <button onClick={generateReport} disabled={reportLoading}
                  style={{width:'100%',padding:'13px',background:'rgba(0,0,0,0.05)',border:'1px solid rgba(0,0,0,0.10)',borderRadius:12,color:'var(--text-primary)',cursor:reportLoading?'not-allowed':'pointer',fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:700,fontSize:'.9rem',opacity:reportLoading?.6:1}}>
...
function ResultView({ result, imgPreview, goals, onBack, onSave, saving, savingTemplate, onSaveTemplate, err, NUTRIENTS, pct }) {
  ...
  const INP = {background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',color:'var(--text-primary)',padding:'7px 9px',fontFamily:"'DM Sans','Tajawal',sans-serif",fontSize:'.8rem',borderRadius:8,outline:'none',transition:'all .2s'}
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14}}>
        <button onClick={onBack} style={{background:'none',border:'none',color:'var(--text-secondary)',cursor:'pointer',fontSize:'1.1rem'}}>←</button>
        <div style={{fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,fontSize:'1.1rem',flex:1}}>{result.meal_name}</div>
        <span style={{background:'rgba(0,0,0,0.06)',border:'1px solid rgba(0,0,0,0.12)',color:'var(--text-primary)',padding:'3px 10px',borderRadius:20,fontSize:'.72rem',fontWeight:700}}>{showCal} kcal</span>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6,marginBottom:12}}>
        {[['البروتين',showProt,'g','#3b82f6'],['الكارب',showCarbs,'g','#f97316'],['الدهن',showFat,'g','#a855f7'],['الألياف',ings.length?r1(ingTotals.fiber_g):r1(result.fiber_g),'g','#22c55e']].map(([l,v,u,col])=>(
          <div key={l} style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:10,padding:'10px 4px',textAlign:'center'}}>
      ...
      {/* Full nutrients */}
      <div style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:12,padding:'12px 14px',marginBottom:12}}>
        <div style={{fontSize:'.6rem',fontWeight:700,letterSpacing:1.5,color:'var(--text-secondary)',marginBottom:10}}>التفاصيل الغذائية الكاملة</div>
        {NUTRIENTS.filter(n=>!['protein_g','carbs_g','fat_g','fiber_g'].includes(n.key)).map((n,i)=>result[n.key]>0&&(
          <div key={n.key} className="mrow">
            <span style={{fontSize:'.78rem',color:'var(--text-secondary)'}}>{n.label}</span>
      ...
      {(ings.length > 0 || result.ingredients?.length > 0) && (
        <div style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:12,padding:'12px 14px',marginBottom:12}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            <div style={{fontSize:'.6rem',fontWeight:700,letterSpacing:1.5,color:'var(--text-secondary)'}}>المكونات</div>
            <button onClick={()=>setEditMode(v=>!v)}
              style={{background:editMode?'rgba(0,0,0,0.08)':'rgba(0,0,0,0.04)',border:'1px solid '+(editMode?'rgba(0,0,0,0.20)':'rgba(0,0,0,0.08)'),borderRadius:7,padding:'4px 10px',color:editMode?'var(--text-primary)':'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontSize:'.72rem',fontWeight:700}}>
          ...
          {!editMode ? (
            /* Read-only view */
            ings.map((ing, i) => (
              <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'7px 0',borderTop:i>0?'1px solid rgba(0,0,0,0.06)':'none'}}>
                <div>
                  <div style={{fontSize:'.84rem',fontWeight:500}}>{ing.name}</div>
                  {ing.portion&&<div style={{fontSize:'.68rem',color:'var(--text-secondary)',marginTop:1}}>{ing.portion}</div>}
                </div>
                <span style={{fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:700,color:'var(--text-primary)',fontSize:'.85rem'}}>{ing.calories} kcal</span>
              </div>
            ))
          ) : (
            /* Edit mode */
            <div>
              {/* Live totals bar */}
              <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:4,marginBottom:10,padding:'8px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:9}}>
                {[['Cal',Math.round(ingTotals.calories),'سعرة','#111111'],['Prot',r1(ingTotals.protein_g),'g','#3b82f6'],['الكارب',r1(ingTotals.carbs_g),'g','#f97316'],['الدهن',r1(ingTotals.fat_g),'g','#a855f7'],['الألياف',r1(ingTotals.fiber_g),'g','#22c55e']].map(([l,v,u,col])=>(
                  <div key={l} style={{textAlign:'center'}}>
                    ...
                    <div style={{fontSize:'.52rem',color:'var(--text-secondary)',marginTop:2,fontWeight:700}}>{l}</div>
                  </div>
                ))}
              </div>
              <div style={{fontSize:'.62rem',color:'var(--text-secondary)',marginBottom:8}}>⚖️ Change gram values to auto-scale nutrition</div>

              {ings.map((ing, i) => (
                <div key={i} style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:10,padding:'9px',marginBottom:7}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:4}}>
                    {[['سعرة','calories'],['P g','protein_g'],['C g','carbs_g'],['F g','fat_g'],['Fi g','fiber_g']].map(([l,k])=>(
                      <div key={k}>
                        <div style={{fontSize:'.52rem',color:'var(--text-secondary)',marginBottom:2,fontWeight:700,textAlign:'center'}}>{l.toUpperCase()}</div>
              ...
              <button onClick={addIng} style={{width:'100%',padding:'8px',background:'transparent',border:'1px dashed rgba(0,0,0,0.12)',borderRadius:9,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontWeight:600,fontSize:'.78rem',marginBottom:4}}>
                + أضف مكوّن
              </button>
            </div>
          )}
      ...
      <div style={{display:'flex',gap:8,marginBottom:8}}>
        <button onClick={onBack} style={{flex:1,padding:'14px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:12,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontWeight:600,fontSize:'.88rem'}}>Re-analyze</button>
        <button onClick={()=>onSave(getFinalResult())} disabled={saving} style={{flex:2,padding:'14px',background:'#111111',border:'none',borderRadius:12,fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,fontSize:'.95rem',color:'#FFFFFF',cursor:saving?'not-allowed':'pointer'}}>
          {saving?'حافظين…':'سجّل الوجبة ✓'}
        </button>
      </div>
      <button onClick={()=>onSaveTemplate(getFinalResult())} disabled={savingTemplate} style={{width:'100%',padding:'11px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:12,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontWeight:600,fontSize:'.82rem'}}>
...
function EditModal({ meal, onSave, onClose, onReanalyze }) {
  ...
  const INP = { background:'rgba(0,0,0,0.04)', border:'1px solid rgba(0,0,0,0.08)', color:'var(--text-primary)', padding:'8px 10px', fontFamily:"'DM Sans','Tajawal',sans-serif", fontSize:'.82rem', borderRadius:8, outline:'none', width:'100%', transition:'all .2s' }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.94)',zIndex:400,display:'flex',alignItems:'flex-end',backdropFilter:'blur(8px)'}}>
      <style>{`*{box-sizing:border-box} input,textarea{background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.08);color:var(--text-primary);padding:9px 12px;font-family:'DM Sans','Tajawal',sans-serif;font-size:.85rem;border-radius:9px;outline:none;width:100%;transition:all .2s} input:focus,textarea:focus{border-color:#111111;background:rgba(0,0,0,0.06)} ::placeholder{color:rgba(0,0,0,0.30)} @keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <div style={{background:'var(--surface)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:'22px 22px 0 0',width:'100%',maxHeight:'92vh',overflowY:'auto',padding:'16px 16px calc(24px + env(safe-area-inset-bottom))'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
          ...
          <button onClick={onClose} style={{background:'rgba(0,0,0,0.05)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:8,padding:'5px 11px',color:'var(--text-secondary)',cursor:'pointer',fontSize:'.78rem',fontFamily:"'DM Sans','Tajawal',sans-serif"}}>إلغاء</button>
        </div>
        ...
        <div style={{display:'flex',background:'rgba(0,0,0,0.05)',borderRadius:10,padding:3,marginBottom:16,gap:3}}>
          {[['ingredients','✏️ Edit Ingredients'],['reanalyze','🔍 Re-analyze with AI']].map(([id,label]) => (
            <button key={id} onClick={() => { setTab(id); setErr('') }}
              style={{flex:1,padding:'9px 8px',background:tab===id?'rgba(0,0,0,0.08)':'transparent',border:'1px solid ' + (tab===id?'rgba(0,0,0,0.20)':'transparent'),borderRadius:8,color:tab===id?'var(--text-primary)':'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontSize:'.76rem',fontWeight:700,transition:'all .15s'}}>
              {label}
            </button>
          ))}
        </div>
        ...
        {tab === 'ingredients' && (
          <div>
            {/* Live totals bar */}
            <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:5,marginBottom:14,padding:'11px 10px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:12}}>
              {[['Cal',Math.round(totals.calories),'سعرة','#111111'],['Prot',r1(totals.protein_g),'g','#3b82f6'],['Carb',r1(totals.carbs_g),'g','#f97316'],['الدهن',r1(totals.fat_g),'g','#a855f7'],['الألياف',r1(totals.fiber_g),'g','#22c55e']].map(([l,v,u,col]) => (
                <div key={l} style={{textAlign:'center'}}>
                  ...
                  <div style={{fontSize:'.55rem',color:'var(--text-secondary)',marginTop:2,fontWeight:700}}>{l.toUpperCase()}</div>
                </div>
              ))}
            </div>
            <div style={{color:'var(--text-secondary)',fontSize:'.72rem',marginBottom:10,lineHeight:1.5}}>
            ...
            {ingredients.map((ing, i) => {
              ...
              return (
              <div key={i} style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:13,padding:'11px',marginBottom:8}}>
                {/* Name + portion + remove */}
                ...
                {/* Auto-scale hint */}
                {hasGrams && <div style={{fontSize:'.62rem',color:'var(--text-secondary)',marginBottom:6,marginTop:-2}}>⚖️ Nutrition auto-scales when you change the gram value</div>}
                {/* Nutrition fields */}
                <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:5}}>
                  {[['كالوري (kcal)','calories'],['بروتين g','protein_g'],['كارب g','carbs_g'],['دهن g','fat_g'],['ألياف g','fiber_g']].map(([label,key]) => (
                    <div key={key}>
                      <div style={{fontSize:'.56rem',color:'var(--text-secondary)',marginBottom:3,fontWeight:700,textAlign:'center'}}>{label.toUpperCase()}</div>
            ...
            <button onClick={addRow} style={{width:'100%',padding:'10px',background:'rgba(0,0,0,0.03)',border:'1px dashed rgba(0,0,0,0.12)',borderRadius:10,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontWeight:600,fontSize:'.82rem',marginBottom:14}}>
              + أضف مكوّن
            </button>
            <button onClick={saveIngredients} disabled={saving} style={{width:'100%',padding:'15px',background:'#111111',border:'none',borderRadius:13,fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,fontSize:'.95rem',color:'#FFFFFF',cursor:saving?'not-allowed':'pointer',transition:'all .2s'}}>
        ...
        {tab === 'reanalyze' && (
          <div>
            <div style={{color:'var(--text-secondary)',fontSize:'.78rem',marginBottom:14,lineHeight:1.55}}>
              Upload a new photo or update the description - AI will recalculate all nutrition data and ingredients.
            </div>
            <div onClick={() => { fileRef.current.removeAttribute('capture'); fileRef.current.click() }}
              style={{border:'2px dashed ' + (imgPreview?'rgba(0,0,0,0.08)':'rgba(0,0,0,0.12)'),borderRadius:14,overflow:'hidden',marginBottom:8,cursor:'pointer',minHeight:imgPreview?0:90,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.05)'}}>
              {imgPreview
                ...
                : <div style={{textAlign:'center',padding:'18px'}}><div style={{fontSize:'1.8rem',marginBottom:4}}>📸</div><div style={{color:'var(--text-secondary)',fontSize:'.8rem'}}>Upload new photo (optional)</div></div>}
            </div>
            <input ref={fileRef} type="file" accept="image/*" style={{display:'none'}} onChange={e => loadImg(e.target.files[0])}/>
            <div style={{display:'flex',gap:6,marginBottom:10}}>
              <button onClick={() => { fileRef.current.removeAttribute('capture'); fileRef.current.click() }} style={{flex:1,padding:'9px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:9,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontSize:'.78rem',fontWeight:600}}>🖼 Gallery</button>
              <button onClick={() => { fileRef.current.setAttribute('capture','environment'); fileRef.current.click() }} style={{flex:1,padding:'9px',background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:9,color:'var(--text-secondary)',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif",fontSize:'.78rem',fontWeight:600}}>📷 Camera</button>
            </div>
            ...
            {/* AI result preview */}
            {reResult && (
              <div style={{background:'var(--card)',border:'1px solid rgba(0,0,0,0.08)',borderRadius:11,padding:'11px 13px',marginBottom:10}}>
                ...
                <div style={{display:'flex',gap:10,fontSize:'.75rem',flexWrap:'wrap'}}>
                  <span style={{color:'var(--text-primary)',fontWeight:800}}>{reResult.total_calories} kcal</span>
                ...
                {reResult.ingredients?.length > 0 && (
                  <div style={{marginTop:8,fontSize:'.72rem',color:'var(--text-secondary)'}}>
                    {reResult.ingredients.map((ing,i) => (
                      <div key={i} style={{padding:'2px 0'}}>{ing.name} - {ing.portion || ''} · {ing.calories} kcal</div>
                    ))}
                  </div>
                )}
                <div style={{fontSize:'.7rem',color:'var(--text-secondary)',marginTop:6}}>✓ Also updated ingredients tab with these values</div>
              </div>
            )}

            {analyzing
              ? <div style={{textAlign:'center',padding:'18px'}}><div style={{width:36,height:36,border:'3px solid rgba(0,0,0,0.10)',borderTopColor:'#111111',borderRadius:'50%',animation:'spin .8s linear infinite',margin:'0 auto 10px'}}/><div style={{color:'var(--text-primary)',fontWeight:600,fontSize:'.88rem'}}>Analyzing nutrition…</div></div>
              : <div style={{display:'flex',gap:8}}>
                  <button onClick={reanalyze} disabled={!imgB64 && !textInput.trim()}
                    style={{flex:1,padding:'13px',background:(!imgB64&&!textInput.trim())?'rgba(0,0,0,0.05)':'rgba(0,0,0,0.08)',border:'1px solid ' + ((!imgB64&&!textInput.trim())?'rgba(0,0,0,0.08)':'rgba(0,0,0,0.18)'),borderRadius:12,color:(!imgB64&&!textInput.trim())?'rgba(0,0,0,0.25)':'var(--text-primary)',cursor:(!imgB64&&!textInput.trim())?'not-allowed':'pointer',fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:700,fontSize:'.88rem'}}>
                    🔍 Analyze
                  </button>
                  {reResult && <button onClick={saveReanalyzed} disabled={saving} style={{flex:1,padding:'13px',background:'#111111',border:'none',borderRadius:12,fontFamily:"'Space Grotesk','Tajawal',sans-serif",fontWeight:800,fontSize:'.88rem',color:'#FFFFFF',cursor:saving?'not-allowed':'pointer'}}>{saving?'جاري الحفظ':'Save ✓'}</button>}
                </div>}
          </div>
        )}
      </div>
