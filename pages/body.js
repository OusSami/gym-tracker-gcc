...
const Tip = ({ active, payload, label }) => {
  ...
  return <div style={{background:'#1a1a1a',border:'1px solid #2a2a2a',borderRadius:8,padding:'8px 12px',fontSize:12,color:'#ECE3CF'}}><div style={{color:'#666',marginBottom:4,fontSize:11}}>{label}</div>{payload.map((p,i)=><div key={i} style={{color:p.color||'#CBA23B'}}>{p.name}: <b>{p.value}</b></div>)}</div>
}
...
export default function BodyAnalysis() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0C0B0D',color:'#ECE3CF',fontFamily:"'DM Sans','Tajawal',sans-serif",maxWidth:700,margin:'0 auto',padding:'0 0 80px'}}>
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .btn{display:block;width:100%;padding:15px;border:none;border-radius:12px;font-family:'Bebas Neue','Noto Kufi Arabic',sans-serif;font-size:1.1rem;letter-spacing:2px;cursor:pointer;transition:all .15s;text-align:center}
        ...
        .btn-sm{background:#161616;border:1px solid #2a2a2a;color:#aaa;font-family:'DM Sans','Tajawal',sans-serif;font-size:.75rem;padding:7px 13px;border-radius:8px;cursor:pointer}.btn-sm:hover{border-color:#555;color:#fff}
      ...
      <div style={{padding:'22px 20px 0',display:'flex',alignItems:'center',gap:12,marginBottom:4}}>
        ...
        <div>
          <div style={{display:'flex',alignItems:'baseline',gap:6}}>
            <span className="bb" style={{fontSize:'1.6rem',color:'#CBA23B'}}>جسمك</span>
        ...
        {tab === 'upload' && (
          <div className="up">
            ...
            <div style={{background:'#0d0d0d',border:'1px solid #1a1a1a',borderRadius:12,padding:'14px 16px',marginBottom:16}}>
              <div className="bb" style={{fontSize:'.72rem',color:'#888',letterSpacing:2,marginBottom:6}}>كيف يعمل</div>
            ...
            <div style={{marginBottom:14}}>
              <div style={{color:'#aaa',fontSize:'.75rem',fontWeight:600,marginBottom:6}}>تاريخ الصورة</div>
              <input type="date" value={analysisDate} max={new Date().toISOString().split('T')[0]}
                onChange={e=>setAnalysisDate(e.target.value)}
                style={{background:'#141414',border:'1px solid #2a2a2a',color:'#ECE3CF',padding:'11px 14px',borderRadius:10,outline:'none',width:'100%',fontSize:'.9rem'}}/>
            ...
            <div style={{marginBottom:12}}>
              <div style={{color:'#aaa',fontSize:'.75rem',fontWeight:600,marginBottom:6}}>FRONT PHOTO <span style={{color:'#CBA23B'}}>*</span></div>
              <div onClick={()=>frontRef.current?.click()}
                ...
                {frontPreview ? (
                  ...
                  <div style={{textAlign:'center',padding:'24px 20px'}}>
                    ...
                    <div className="bb" style={{color:'#555',fontSize:'.9rem',letterSpacing:2}}>الصورة الأمامية</div>
                    <div style={{color:'#3a3a3a',fontSize:'.72rem',marginTop:4}}>أمامياً · وجهك للكاميرا · يداك جانبك</div>
                  </div>
                )}
              </div>
              ...
              <div onClick={()=>backRef.current?.click()}
                ...
                {backPreview ? (
                  ...
                  <div style={{textAlign:'center',padding:'20px'}}>
                    ...
                    <div className="bb" style={{color:'#444',fontSize:'.85rem',letterSpacing:2}}>الصورة الخلفية</div>
                    <div style={{color:'#2a2a2a',fontSize:'.72rem',marginTop:3}}>تكشف الظهر والعريضة والأرداف</div>
                  </div>
                )}
              </div>
            ...
            {analyzing ? (
              <div style={{textAlign:'center',padding:'30px'}}>
                <div style={{width:48,height:48,border:'3px solid #1e1e1e',borderTopColor:'#CBA23B',borderRadius:'50%',animation:'spin .8s linear infinite',margin:'0 auto 16px'}}/>
                <div className="bb" style={{color:'#CBA23B',fontSize:'1.4rem',letterSpacing:3}}>ANALYZING…</div>
                <div style={{color:'#555',fontSize:'.82rem',marginTop:6}}>جاري تحليل جسمك — يستغرق أقل من 30 ثانية</div>
              </div>
            ) : (
              ...
              <>
                ...
                {analyses.length >= 2 && (
                  <div className="card" style={{marginBottom:16}}>
                    <div className="bb" style={{fontSize:'.85rem',color:'#ccc',letterSpacing:2,marginBottom:14}}>تطور جسمك</div>
                    <ResponsiveContainer width="100%" height={180}>
                      <LineChart data={historyData} margin={{top:5,right:5,bottom:0,left:0}}>
                        ...
                        <Tooltip content={<Tip/>}/>
                        <Line type="monotone" dataKey="Overall" stroke="#CBA23B" strokeWidth={2.5} dot={{fill:'#CBA23B',r:4,strokeWidth:0}}/>
                    ...
                    <div style={{display:'flex',gap:14,justifyContent:'center',marginTop:10}}>
                      {[['Overall','#CBA23B'],['Definition','#3b82f6'],['Symmetry','#22c55e']].map(([l,c])=>(
                  ...
                  <div style={{display:'flex',gap:8,marginBottom:14,overflowX:'auto',paddingBottom:4}}>
                    {analyses.map((a,i)=>(
                      <button key={a.id} onClick={()=>setSelected(a)}
                        style={{flexShrink:0,padding:'6px 12px',background:selected?.id===a.id?'rgba(232,255,71,.1)':'#111',border:`1px solid ${selected?.id===a.id?'#CBA23B':'#222'}`,borderRadius:8,color:selected?.id===a.id?'#CBA23B':'#888',fontSize:'.78rem',cursor:'pointer',fontFamily:'DM Sans,sans-serif',whiteSpace:'nowrap'}}>
                ...
                {selected && (
                  <div className="up">
                    ...
                    <div style={{display:'grid',gridTemplateColumns:selected.image_url?'1fr 1fr':'1fr',gap:14,marginBottom:16}}>
                        ...
                        <div>
                          <div style={{display:'flex',justifyContent:'flex-end',marginBottom:6}}>
                            <button onClick={()=>setHiddenPhotos(p=>({...p,[selected.id]:!p[selected.id]}))}
                              style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:8,color:'var(--text-muted)',fontSize:'.72rem',padding:'4px 10px',cursor:'pointer',fontFamily:"'DM Sans','Tajawal',sans-serif"}}>
                      ...
                      <div>
                        ...
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                          <ScoreRing value={selected.overall_score} label="OVERALL" color="#CBA23B" size={76}/>
                    ...
                    <div className="card" style={{marginBottom:12}}>
                      <div className="bb" style={{fontSize:'.8rem',color:'#ccc',letterSpacing:2,marginBottom:10}}>تطور العضلات</div>
                    ...
                    {/* Recommendations */}
                    <div className="card" style={{marginBottom:12}}>
                      <div className="bb" style={{fontSize:'.8rem',color:'#ccc',letterSpacing:2,marginBottom:12}}>توصياتنا لك</div>
                      {selected.recommendations?.map((r,i)=>(
                        <div key={i} style={{display:'flex',gap:12,marginBottom:12,alignItems:'flex-start'}}>
                          <div style={{width:24,height:24,borderRadius:'50%',background:'#CBA23B',color:'#0C0B0D',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'.72rem',fontWeight:700,flexShrink:0,marginTop:1}}>{i+1}</div>
                    ...
                    {selected.full_report && (
                      <div className="card" style={{marginBottom:12}}>
                        <div className="bb" style={{fontSize:'.8rem',color:'#ccc',letterSpacing:2,marginBottom:12}}>التقرير الكامل</div>
...
function Loader() {
  return <div style={{minHeight:'100vh',background:'#0C0B0D',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{width:32,height:32,border:'3px solid #1e1e1e',borderTopColor:'#CBA23B',borderRadius:'50%',animation:'spin .8s linear infinite'}}/><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>
}
