/**
 * GET /api/packages/meal-plan-v2?userId=xxx&day=N
 *
 * Phase 3 (STAGING) — multi-item slot composer.
 * Drop-in for meal-plan.js once approved; live API is UNTOUCHED until then.
 *
 * Key differences from v1:
 *   - Fetches ALL dishes + ALL extras in 2 queries upfront (vs N_slots × up-to-5 per slot)
 *   - Each slot can be solo_dish | dish_extra | dish_2extra | solo_extra | multi_extra
 *   - Response extends v1 shape: `food` still present (primary item, backward-compat),
 *     plus `items[]` array with per-item breakdown and `shape` string
 */
...
function buildSlotEntry(mealTime, result, targetCal) {
  ...
  return {
    meal_time: mealTime,
    // v1-compatible primary food entry (calorie reflects slot total)
    food: primary ? {
...
export default async function handler(req, res) {
  ...
  // 2 queries instead of up-to-20 in v1
  ...
  const recentIds  = new Set()  // Phase 3 baseline; future: seed from session history
  const plan       = []
  let dayDatesUsed = 0
  ...
  for (let mealIdx = 0; mealIdx < MEAL_STRUCTURE.length; mealIdx++) {
    ...
    if (!result || !result.items.length) {
      ...
      console.error(`[meal-plan-v2] WARN: no result for required slot ${mealTime}`)
      continue
    }
  ...
  // Emergency fallback for missing required meals (identical logic to v1)
  ...
  for (const { time: required, pct } of MEAL_STRUCTURE.filter(m => m.required)) {
    ...
    console.error(`[meal-plan-v2] EMERGENCY: required meal ${required} missing`)
  ...
  console.log('[meal-plan-v2] Plan meals:', plan.map(p => p.meal_time))
