...
export default function Landing() {
  ...
  const btn = {display:'flex',alignItems:'center',justifyContent:'center',gap:10,background:'#111111',color:'#ffffff',padding:'17px 36px',borderRadius:14,fontFamily:"'Tajawal',sans-serif",fontWeight:900,fontSize:'1.05rem',border:'none',cursor:'pointer',textDecoration:'none',width:'100%',maxWidth:420,margin:'0 auto',boxShadow:'0 8px 40px rgba(0,0,0,0.12)',transition:'all .2s'}
    ...
    <>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        body{background:var(--surface);color:var(--text-primary);font-family:'Tajawal',sans-serif;direction:rtl;overflow-x:hidden;line-height:1.65}
        a{text-decoration:none;color:inherit}
        .mx{max-width:700px;margin:0 auto;padding:0 20px}
        .sec{padding:72px 0}
        .dv{height:1px;background:linear-gradient(90deg,transparent,rgba(0,0,0,0.10),transparent)}
        .card{background:var(--card);border:1px solid rgba(0,0,0,0.10);border-radius:20px;padding:28px}
        ...
        .lbl{font-size:.68rem;font-weight:700;letter-spacing:3.5px;text-transform:uppercase;color:var(--text-secondary)}
        .muted{color:var(--text-secondary);font-size:.88rem;line-height:1.75}
        .gold{color:var(--text-primary)}
        @keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}
        @keyframes bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}}
        .glow{width:7px;height:7px;border-radius:50%;background:rgba(0,0,0,0.30);display:inline-block;animation:pulse 2s infinite}
        .sympt{display:flex;align-items:flex-start;gap:14px;padding:18px 20px;border-radius:14px;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.08);margin-bottom:10px;transition:border-color .2s}
        .sympt:hover{border-color:rgba(0,0,0,0.15)}
        ...
        .ph2{background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.12)}
        ...
        .phitem::before{content:'✓';color:var(--text-primary);font-weight:900;flex-shrink:0;margin-top:1px}
        .bene{display:flex;align-items:flex-start;gap:16px;padding:20px 22px;background:var(--card);border:1px solid rgba(0,0,0,0.08);border-radius:16px;margin-bottom:10px;transition:all .2s}
        .bene:hover{border-color:rgba(0,0,0,0.18);transform:translateX(-3px)}
        .bnum{font-size:1.7rem;font-weight:900;color:rgba(0,0,0,0.10);font-family:'IBM Plex Mono',monospace;flex-shrink:0;line-height:1;min-width:34px}
        ...
        .ctbl th,.ctbl td{padding:13px 11px;font-size:.8rem;border-bottom:1px solid rgba(0,0,0,0.06);text-align:center}
        .ctbl th{font-weight:700;font-size:.7rem;letter-spacing:1px;color:var(--text-secondary)}
        .ctbl td:first-child{text-align:right;font-weight:600}
        .ctbl tr:last-child td{border-bottom:none}
        .cus{color:var(--text-primary);font-weight:800}
        .tst{background:var(--card);border:1px solid rgba(0,0,0,0.08);border-radius:18px;padding:24px;margin-bottom:11px;position:relative}
        .tst::before{content:'"';position:absolute;top:10px;inset-inline-start:16px;font-size:5rem;line-height:1;color:rgba(0,0,0,0.04);font-family:Georgia,serif;pointer-events:none}
        .sbar{position:fixed;bottom:0;inset-inline:0;z-index:200;background:var(--surface);backdrop-filter:blur(20px);border-top:1px solid rgba(0,0,0,0.10);padding:14px 20px;display:flex;align-items:center;justify-content:space-between;gap:14px}
      ...
      <section className="sec" style={{position:'relative',overflow:'hidden',paddingTop:0,paddingBottom:80}}>
        ...
        <div className="mx" style={{textAlign:'center',position:'relative'}}>
          ...
          <p className="rv" style={{fontSize:'1.05rem',color:'var(--text-secondary)',maxWidth:480,margin:'0 auto 10px',lineHeight:1.7}}>
            المشكلة مو في إرادتك.<br/>المشكلة إن ما عندك <strong style={{color:'var(--text-primary)'}}>نظام حقيقي يحاسبك ويتكيف معك.</strong>
          </p>
          <div className="rv" style={{display:'inline-flex',alignItems:'center',gap:14,background:'rgba(0,0,0,0.04)',border:'1px solid rgba(0,0,0,0.10)',borderRadius:14,padding:'14px 22px',margin:'26px auto'}}>
            <div>
          ...
          <div className="rv">
            ...
            <div style={{fontSize:'.73rem',color:'var(--text-secondary)',marginTop:12}}>✓ برنامج مخصص بالذكاء · ✓ أكل خليجي · ✓ دخول فوري</div>
          </div>
          <div className="rv" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginTop:48}}>
            {[['21','يوم للتغيير'],['100%','مخصص لجسمك'],['GCC','مطبخ خليجي']].map(([v,l])=>(
              <div key={l} className="card" style={{padding:'18px 12px',textAlign:'center'}}>
                <div style={{fontSize:'1.7rem',fontWeight:900,color:'var(--text-primary)',lineHeight:1,fontFamily:"'IBM Plex Mono',monospace"}}>{v}</div>
                <div style={{fontSize:'.7rem',color:'var(--text-secondary)',marginTop:5}}>{l}</div>
              </div>
            ))}
          </div>

          {/* Hero avatars */}
          <div className="rv" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:0,marginTop:40,borderRadius:24,overflow:'hidden',border:'1px solid rgba(0,0,0,0.10)'}}>
          ...
          {[
            ...
            <div key={title} className="sympt rv">
              ...
              <div>
                ...
                <div style={{fontSize:'.8rem',color:'var(--text-secondary)',lineHeight:1.55}}>{desc}</div>
              </div>
            </div>
          ))}
          {/* Before/after transformation */}
          <div className="rv" style={{marginTop:32,borderRadius:20,overflow:'hidden',border:'1px solid rgba(0,0,0,0.10)'}}>
            <img src="/landing-transform.webp" alt="قبل وبعد" style={{width:'100%',height:'auto',display:'block'}}/>
          </div>

          <blockquote className="rv" style={{borderRight:`3px solid rgba(0,0,0,0.20)`,padding:'14px 20px',marginTop:28,fontStyle:'italic',color:'var(--text-secondary)',fontSize:'.9rem',lineHeight:1.7}}>
        ...
        <div className="mx">
          ...
          <div className="card rv" style={{marginBottom:16}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              {[['01','حماس البداية','تشتري ملابس، تتابع محتوى، تبدأ بطاقة عالية.','rgba(0,0,0,0.10)'],['02','أول عائق','يوم مشغول، مناسبة عيلية، تعب — تغيب يومين.','rgba(0,0,0,0.10)'],['03','الشعور بالفشل','"خربت كل شيء — بكرة أبدأ من الصفر."','rgba(0,0,0,0.10)'],['04','توقف كامل','وتكرر نفس الدائرة الشهر القادم.','rgba(239,68,68,0.25)']].map(([n,t,d,c])=>(
                <div key={n} style={{background:'var(--card)',border:`1px solid ${n==='04'?'rgba(239,68,68,0.2)':'rgba(0,0,0,0.08)'}`,borderRadius:14,padding:20}}>
                  <div style={{fontSize:'1.8rem',fontWeight:900,color:c,fontFamily:"'IBM Plex Mono',monospace",lineHeight:1,marginBottom:8}}>{n}</div>
                  <div style={{fontWeight:700,fontSize:'.9rem',marginBottom:5,color:n==='04'?'#ef4444':'var(--text-primary)'}}>{t}</div>
      ...
      {/* ── SOLUTION ── */}
      <section className="sec" style={{background:'rgba(0,0,0,0.02)'}}>
        <div className="mx">
          ...
          <p className="muted rv" style={{marginBottom:36}}>مو مجرد تطبيق تتبع. <strong style={{color:'var(--text-primary)'}}>مدرب ذكي يعرف جسمك، يفهم أكلك الخليجي، ويبني خطتك يومياً</strong> بناءً على ما حصل بالفعل.</p>

          {/* App mockup */}
          <div className="rv" style={{textAlign:'center',marginBottom:40,borderRadius:24,overflow:'hidden'}}>
            <img src="/landing-app-mockup.webp" alt="تطبيق GYM TRACKER GCC" style={{width:'75%',maxWidth:320,height:'auto',display:'inline-block',filter:'drop-shadow(0 20px 60px rgba(0,0,0,0.15))'}}/>
          </div>
          ...
          {[
            ...
            ['الأيام 8 – 14 ← المرحلة الأهم','ph2','rgba(0,0,0,0.06)','var(--text-primary)','النظام يتكيف معك — لا يعاقبك','فاتك يوم؟ عندك مناسبة؟ النظام يعدّل ويكمل.',['محاسبة يومية ذكية — تقدير لا مثالية','مدرب ذكي بالعربي يرد فوراً','الخطة تتكيف مع حياتك']],
      ...
      <section className="sec">
        <div className="mx">
          ...
          {[
            ...
            <div key="food" className="rv" style={{borderRadius:20,overflow:'hidden',margin:'4px 0 10px',border:'1px solid rgba(0,0,0,0.10)'}}>
            ...
            <div key={n} className="bene rv">
              ...
              <div>
                ...
                <div style={{fontSize:'.82rem',color:'var(--text-secondary)',lineHeight:1.65}}>{d}</div>
              </div>
            </div>
          ))}
        </div>
            ...
            <table className="ctbl">
              ...
              <tbody>
                {[['برنامج لجسمك تحديداً','✗','✓','✓'],['أكل خليجي بالماكروز','✗','نادراً','✓'],['متاح 24/7 بالعربي','✗','✗','✓'],['يتكيف مع انقطاعك','✗','أحياناً','✓ دائماً'],['التكلفة الشهرية','مجاني','+1500 ر.س','149 ر.س']].map(([f,a,b,c])=>(
                  <tr key={f}><td>{f}</td><td style={{color:'var(--text-secondary)'}}>{a}</td><td style={{color:'var(--text-secondary)'}}>{b}</td><td className="cus">{c}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
        ...
        <div className="mx">
          ...
          {[
            ...
            <div key={name} className="tst rv">
              <div style={{color:'var(--text-primary)',fontSize:'.8rem',marginBottom:8}}>★★★★★</div>
              <p style={{fontSize:'.87rem',lineHeight:1.8,marginBottom:14}}>{text}</p>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                <div style={{width:36,height:36,borderRadius:'50%',background:'rgba(0,0,0,0.06)',border:'1px solid rgba(0,0,0,0.12)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,fontSize:'.88rem',color:'var(--text-primary)',flexShrink:0}}>{av}</div>
                <div>
                  <div style={{fontWeight:700,fontSize:'.83rem'}}>{name}</div>
                  <div style={{fontSize:'.7rem',color:'var(--text-secondary)'}}>{time}</div>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="sec" id="checkout">
        <div className="mx">
          <div className="card rv" style={{border:'1px solid rgba(0,0,0,0.12)',background:'rgba(0,0,0,0.02)',textAlign:'center'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:7,fontSize:'.73rem',color:'var(--text-secondary)',marginBottom:24}}>
            ...
            <div style={{background:'linear-gradient(135deg,rgba(34,197,94,.07),rgba(0,0,0,0.02))',border:'1px solid rgba(34,197,94,.18)',borderRadius:18,padding:24}}>
              <div style={{fontSize:'1.8rem',marginBottom:8}}>🛡️</div>
              <div style={{fontWeight:800,fontSize:'.95rem',marginBottom:6}}>ضمان 7 أيام كامل</div>
              <div style={{fontSize:'.82rem',color:'var(--text-secondary)'}}>جرّب 7 أيام. إذا ما شفت فرقاً حقيقياً — نرد لك المبلغ كاملاً بدون أسئلة.</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="sec">
        <div className="mx">
          ...
          {faqs.map(([q,a],i)=>(
            <div key={i} style={{borderBottom:'1px solid rgba(0,0,0,0.08)',padding:'20px 0'}} className="rv">
              <div onClick={()=>setOpenFaq(openFaq===i?null:i)} style={{fontWeight:700,fontSize:'.93rem',display:'flex',justifyContent:'space-between',alignItems:'center',cursor:'pointer',gap:14}}>
                <span>{q}</span>
                <span style={{color:'var(--text-primary)',fontSize:'1.3rem',transition:'transform .3s',transform:openFaq===i?'rotate(45deg)':'none',flexShrink:0,lineHeight:1}}>+</span>
              </div>
              {openFaq===i&&<div style={{fontSize:'.84rem',color:'var(--text-secondary)',lineHeight:1.8,marginTop:12}}>{a}</div>}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="sec" style={{paddingBottom:120}}>
        <div className="mx" style={{textAlign:'center'}}>
          ...
          <p style={{fontSize:'.72rem',color:'var(--text-secondary)',marginTop:12}}>ضمان 7 أيام · إلغاء في أي وقت · دخول خلال 24 ساعة</p>
        </div>
      </section>
      ...
      <footer style={{borderBlockStart:'1px solid rgba(0,0,0,0.08)',padding:'28px 20px',textAlign:'center'}}>
        ...
        <div style={{fontSize:'.68rem',color:'rgba(0,0,0,0.25)',marginTop:10}}>© 2026 GYM TRACKER GCC. جميع الحقوق محفوظة.</div>
      </footer>
