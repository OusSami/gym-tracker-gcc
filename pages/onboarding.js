...
function SilhouetteMale({ filled }) {
  return (
    <svg viewBox="0 0 80 140" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:52,height:92}}>
      <ellipse cx="40" cy="18" rx="13" ry="14" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
      <path d="M25 32 Q18 42 18 65 L25 65 L25 105 L35 105 L35 70 L45 70 L45 105 L55 105 L55 65 L62 65 Q62 42 55 32 Q47 28 40 28 Q33 28 25 32Z" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
    </svg>
  )
}
function SilhouetteMaleBelly({ filled }) {
  return (
    <svg viewBox="0 0 90 140" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:60,height:92}}>
      <ellipse cx="45" cy="18" rx="14" ry="14" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
      <path d="M28 32 Q18 42 16 58 Q14 72 20 78 Q26 84 45 84 Q64 84 70 78 Q76 72 74 58 Q72 42 62 32 Q53 28 45 28 Q37 28 28 32Z" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
      <path d="M20 78 L22 105 L33 105 L34 75" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
      <path d="M70 78 L68 105 L57 105 L56 75" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
    </svg>
  )
}
function SilhouetteFemale({ filled }) {
  return (
    <svg viewBox="0 0 80 140" fill="none" xmlns="http://www.w3.org/2000/svg" style={{width:52,height:92}}>
      <ellipse cx="40" cy="17" rx="12" ry="13" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
      <path d="M27 30 Q20 40 20 55 Q20 62 27 66 Q34 70 40 70 Q46 70 53 66 Q60 62 60 55 Q60 40 53 30 Q46 26 40 26 Q34 26 27 30Z" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
      <path d="M24 66 Q18 72 18 82 Q18 92 28 95 Q34 97 40 97 Q46 97 52 95 Q62 92 62 82 Q62 72 56 66" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
      <path d="M28 95 L26 110 L36 110 L38 80" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
      <path d="M52 95 L54 110 L44 110 L42 80" fill={filled?G+'88':'rgba(255,255,255,0.15)'}/>
    </svg>
  )
}
    ...
    <div style={{ minHeight:'100vh', background:'#09090B', color:'#ECE3CF', fontFamily:F, direction:'rtl' }}>
      <style>{`
        * { box-sizing:border-box }
        ...
        input:focus { outline:none; border-color:rgba(203,162,59,0.5)!important }
      `}</style>
        ...
        {step > 0 && step < maxStep && (
          <div style={{ padding:'20px 0 0', flexShrink:0 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
              <button onClick={()=>setStep(s=>s-1)} style={{ background:'none',border:'none',color:'rgba(255,255,255,0.4)',cursor:'pointer',fontFamily:F,fontSize:'.85rem',padding:'4px 8px' }}>← رجوع</button>
              <div style={{ fontSize:'.72rem', color:'rgba(255,255,255,0.3)', fontFamily:F }}>{step} من {maxStep-1}</div>
            </div>
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', paddingTop:step===0?60:32 }}>
          ...
          {step === 0 && (
            <div className="fu" style={{ textAlign:'center' }}>
              ...
              <p style={{ fontSize:'.9rem', color:'rgba(255,255,255,0.5)', lineHeight:1.8, marginBottom:36, maxWidth:320, margin:'0 auto 36px' }}>
                دقيقتين ونجهّزلك برنامج تمرين وأكل مخصص لجسمك وحياتك في الخليج.
              </p>
              <button onClick={()=>setStep(1)} style={{ width:'100%', background:G, color:'#09090B', border:'none', borderRadius:16, padding:'17px', fontFamily:F, fontWeight:900, fontSize:'1.05rem', cursor:'pointer', boxShadow:`0 6px 28px rgba(203,162,59,0.35)` }}>
                يلا نبدأ 🔥
              </button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="fu">
              ...
              <p style={{ fontSize:'.83rem', color:'rgba(255,255,255,0.4)', marginBottom:28 }}>{STEPS[2].sub}</p>
              <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                {[
                  ...
                  <div key={f.key}>
                    <label style={{ display:'block', fontSize:'.78rem', fontWeight:700, color:'rgba(255,255,255,0.5)', marginBottom:8 }}>{f.label}</label>
                    <div style={{ display:'flex', gap:10, alignItems:'center' }}>
                      <input type={f.type} inputMode="numeric" placeholder={f.placeholder} value={form[f.key]}
                        onChange={e=>set(f.key, e.target.value)}
                        style={{ flex:1, background:'rgba(255,255,255,0.05)', border:`1px solid ${form[f.key]?'rgba(203,162,59,0.35)':'rgba(255,255,255,0.1)'}`, color:'#ECE3CF', padding:'14px 18px', borderRadius:14, fontFamily:F, fontSize:'1.1rem', fontWeight:700, direction:'ltr', textAlign:'center', transition:'border .2s' }}
                      />
                      <div style={{ fontWeight:700, color:'rgba(255,255,255,0.4)', minWidth:28, textAlign:'right' }}>{f.unit}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {step === 3 && (
            <div className="fu">
              ...
              <p style={{ fontSize:'.83rem', color:'rgba(255,255,255,0.4)', marginBottom:28 }}>{STEPS[3].sub}</p>
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {(getContent(form.sex || 'male').goals).map(g=>(
                  <div key={g.id} onClick={()=>{set('goal',g.id); setTimeout(()=>setStep(4),260)}}
                    style={{ display:'flex', alignItems:'center', gap:14, padding:'18px 20px', background:form.goal===g.id?'rgba(203,162,59,0.1)':'rgba(255,255,255,0.03)', border:`1px solid ${form.goal===g.id?'rgba(203,162,59,0.4)':'rgba(255,255,255,0.08)'}`, borderRadius:18, cursor:'pointer', transition:'all .2s' }}>
                    <div style={{ fontSize:'1.8rem', flexShrink:0 }}>{g.icon}</div>
                    <div>
                      <div style={{ fontWeight:800, fontSize:'.95rem', color:form.goal===g.id?G:'#ECE3CF', marginBottom:3 }}>{g.label}</div>
                      <div style={{ fontSize:'.76rem', color:'rgba(255,255,255,0.4)' }}>{g.sub}</div>
                    </div>
                    {form.goal===g.id && <div style={{ marginRight:'auto', width:20, height:20, borderRadius:'50%', background:G, display:'flex', alignItems:'center', justifyContent:'center' }}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#000" strokeWidth="3.5" strokeLinecap="round"><path d="M20 6L9 17l-5-5"/></svg></div>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {step === 4 && (
            <div className="fu">
              ...
              <p style={{ fontSize:'.83rem', color:'rgba(255,255,255,0.4)', marginBottom:28 }}>{STEPS[4].sub}</p>

              <div style={{ marginBottom:28 }}>
                <div style={{ fontSize:'.78rem', fontWeight:700, color:'rgba(255,255,255,0.5)', marginBottom:12 }}>كم يوم في الأسبوع؟</div>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
                  {[3,4,5].map(d=>(
                    <div key={d} onClick={()=>set('days',d)}
                      style={{ padding:'16px', textAlign:'center', background:form.days===d?'rgba(203,162,59,0.12)':'rgba(255,255,255,0.03)', border:`1px solid ${form.days===d?'rgba(203,162,59,0.4)':'rgba(255,255,255,0.08)'}`, borderRadius:14, cursor:'pointer', transition:'all .2s' }}>
                      <div style={{ fontWeight:900, fontSize:'1.4rem', color:form.days===d?G:'#ECE3CF', fontFamily:'monospace', lineHeight:1 }}>{d}</div>
                      <div style={{ fontSize:'.7rem', color:'rgba(255,255,255,0.4)', marginTop:4 }}>يوم</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div style={{ fontSize:'.78rem', fontWeight:700, color:'rgba(255,255,255,0.5)', marginBottom:12 }}>كم دقيقة لكل تمرين؟</div>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
                  {[{v:15,l:'15 دقيقة',s:'سريع'},{v:30,l:'30 دقيقة',s:'متوسط'},{v:45,l:'45 دقيقة',s:'كامل'}].map(t=>(
                    <div key={t.v} onClick={()=>set('time',t.v)}
                      style={{ padding:'16px 10px', textAlign:'center', background:form.time===t.v?'rgba(203,162,59,0.12)':'rgba(255,255,255,0.03)', border:`1px solid ${form.time===t.v?'rgba(203,162,59,0.4)':'rgba(255,255,255,0.08)'}`, borderRadius:14, cursor:'pointer', transition:'all .2s' }}>
                      <div style={{ fontWeight:800, fontSize:'.88rem', color:form.time===t.v?G:'#ECE3CF' }}>{t.l}</div>
                      <div style={{ fontSize:'.68rem', color:'rgba(255,255,255,0.35)', marginTop:3 }}>{t.s}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {step === 5 && (
            <div className="fu">
              ...
              <p style={{ fontSize:'.83rem', color:'rgba(255,255,255,0.4)', marginBottom:8 }}>اختر كل ما ينطبق عليك — سنعدّل البرنامج والأكل بناءً على وضعك.</p>
                ...
                {HEALTH_OPTIONS.filter(opt => opt.id !== 'pregnancy' || form.sex === 'female').map(opt => {
                  ...
                  return (
                    <div key={opt.id} onClick={() => {
                      ...
                      style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 16px', background:selected?(isBlocked?'rgba(239,68,68,0.1)':'rgba(203,162,59,0.1)'):'rgba(255,255,255,0.03)', border:`1px solid ${selected?(isBlocked?'rgba(239,68,68,0.4)':'rgba(203,162,59,0.4)'):'rgba(255,255,255,0.08)'}`, borderRadius:14, cursor:'pointer', transition:'all .15s' }}>
                      <div style={{ fontSize:'1.2rem', flexShrink:0 }}>{opt.icon}</div>
                      <div style={{ flex:1, fontWeight:600, fontSize:'.88rem', color:selected?(isBlocked?'#ef4444':'#CBA23B'):'#ECE3CF' }}>{opt.label}</div>
                      <div style={{ width:18, height:18, borderRadius:4, border:`2px solid ${selected?(isBlocked?'#ef4444':'#CBA23B'):'rgba(255,255,255,0.2)'}`, background:selected?(isBlocked?'#ef4444':'#CBA23B'):'transparent', flexShrink:0 }}/>
                    </div>
                  )
                })}
              </div>
              ...
              {(healthConditions.includes('heart') || healthConditions.includes('pregnancy')) && (
                <div style={{ background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:14, padding:'16px', marginBottom:16 }}>
                  ...
                  <div style={{ fontSize:'.85rem', color:'rgba(255,255,255,0.7)', lineHeight:1.7 }}>
          ...
          {step === 7 && bmi && (
            <div className="fu">
              <div style={{ textAlign:'center', marginBottom:24 }}>
                ...
                <p style={{ fontSize:'.82rem', color:'rgba(255,255,255,0.4)' }}>هذه نتائج تحليلك الأولي</p>
              </div>

              <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(203,162,59,0.15)', borderRadius:20, padding:22, marginBottom:20 }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                  {[
                    ...
                    <div key={l} style={{ background:'rgba(0,0,0,0.3)', borderRadius:14, padding:'14px', textAlign:'center' }}>
                      <div style={{ fontSize:'.65rem', color:'rgba(255,255,255,0.35)', marginBottom:6, fontFamily:F }}>{l}</div>
                ...
                {form.goal==='lose' && calTarget && form.weight && (
                  <div style={{ marginTop:16, background:'rgba(34,197,94,0.06)', border:'1px solid rgba(34,197,94,0.2)', borderRadius:12, padding:'12px 16px', fontSize:'.82rem', color:'rgba(255,255,255,0.6)', lineHeight:1.7 }}>
              ...
              <button onClick={save} disabled={saving}
                style={{ width:'100%', background:G, color:'#09090B', border:'none', borderRadius:16, padding:'17px', fontFamily:F, fontWeight:900, fontSize:'1.05rem', cursor:'pointer', boxShadow:`0 6px 28px rgba(203,162,59,0.35)` }}>
