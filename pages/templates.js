...
export default function TemplatesPage() {
  ...
  const INP = { background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.10)', color: 'var(--text-primary)', padding: '11px 14px', fontFamily: "'DM Sans','Tajawal',sans-serif", fontSize: '.9rem', borderRadius: 11, outline: 'none', width: '100%' }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--surface)', color: 'var(--text-primary)' }}>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '16px 16px calc(90px + env(safe-area-inset-bottom))' }}>
        ...
        {/* Template list */}
        {loading ? (
          <div style={{ textAlign: 'center', padding: 40 }}><div style={{ width: 32, height: 32, border: '3px solid rgba(0,0,0,0.08)', borderTopColor: '#111111', borderRadius: '50%', animation: 'spin .8s linear infinite', margin: '0 auto' }} /></div>
        ) : templates.length === 0 ? (
          ...
          <div key={t.id} style={{ background: 'var(--card)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 16, padding: '14px', marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginInlineStart: 8 }}>
                <button onClick={() => openEdit(t)} style={{ padding: '6px 10px', background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 8, color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '.75rem' }}>✏️</button>
            ...
            {(t.exercises || []).length > 0 && (
              <div style={{ marginBottom: 10 }}>
                {t.exercises.map((ex, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                    ...
                    <span style={{ fontSize: '.7rem', color: 'var(--text-secondary)' }}>{ex.sets_target} sets</span>
                  </div>
                ))}
              </div>
            )}
            <button onClick={() => startFromTemplate(t)}
              style={{ width: '100%', padding: '11px', background: '#111111', border: 'none', borderRadius: 9999, fontFamily: "'Space Grotesk','Tajawal',sans-serif", fontWeight: 800, fontSize: '.88rem', color: '#FFFFFF', cursor: 'pointer' }}>
              ▶ Start This Workout
            </button>
          </div>
        ))}
      ...
      {/* Edit / New modal */}
      {editing !== null && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--surface)', zIndex: 200, overflowY: 'auto' }}>
          <div style={{ maxWidth: 480, margin: '0 auto', padding: '20px 16px 100px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button onClick={() => setEditing(null)} style={{ background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 9, padding: '7px 14px', color: 'var(--text-secondary)', cursor: 'pointer', fontFamily: "'DM Sans','Tajawal',sans-serif", fontWeight: 700 }}>إلغاء</button>
            </div>

            {/* Name */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: '.65rem', fontWeight: 700, letterSpacing: 1.5, color: 'var(--text-secondary)', marginBottom: 6 }}>اسم القالب</div>
              <input style={INP} placeholder="مثال: يوم الدفع، يوم الأرجل…" value={tName} onChange={e => setTName(e.target.value)} />
            </div>

            {/* Muscles */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: '.65rem', fontWeight: 700, letterSpacing: 1.5, color: 'var(--text-secondary)', marginBottom: 8 }}>العضلات المستهدفة</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                {ALL_MUSCLES.map(gid => {
                  ...
                  return (
                    <div key={gid} style={{ borderRadius: 10, overflow: 'hidden', border: '1px solid ' + (active ? col + '44' : 'rgba(0,0,0,0.08)'), background: active ? col + '09' : 'var(--card)', transition: 'all .15s' }}>
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', flex: 1, cursor: 'pointer', minWidth: 0 }} onClick={() => toggleMuscle(gid)}>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontFamily: "'Space Grotesk','Tajawal',sans-serif", fontWeight: 700, fontSize: '.88rem', color: active ? col : 'var(--text-secondary)' }}>{gid}</div>
                        ...
                        {subs.length > 0 && (
                          <button onClick={() => setExpandedGroup(isOpen ? null : gid)}
                            style={{ padding: '0 13px', alignSelf: 'stretch', background: 'none', border: 'none', borderInlineStart: '1px solid ' + (active ? col + '25' : 'rgba(0,0,0,0.08)'), color: isOpen ? col : 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                      ...
                      {isOpen && subs.length > 0 && (
                        <div style={{ padding: '6px 14px 10px', borderTop: '1px solid ' + col + '18', background: 'rgba(0,0,0,0.03)' }}>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                            {subs.map(sub => {
                              const sel = tMuscles.includes(sub)
                              return <button key={sub} onClick={() => toggleSub(gid, sub)}
                                style={{ padding: '4px 11px', background: sel ? col + '22' : 'rgba(0,0,0,0.04)', border: '1px solid ' + (sel ? col : 'rgba(0,0,0,0.08)'), borderRadius: 20, color: sel ? col : 'var(--text-secondary)', cursor: 'pointer', fontFamily: "'DM Sans','Tajawal',sans-serif", fontSize: '.73rem', fontWeight: sel ? 700 : 400, transition: 'all .12s' }}>{sub}</button>
                            })}
                          </div>
                        </div>
                      )}
            ...
            {/* Exercises */}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: '.65rem', fontWeight: 700, letterSpacing: 1.5, color: 'var(--text-secondary)', marginBottom: 8 }}>التمارين</div>
              {tExercises.map((ex, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, marginBottom: 5 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <button onClick={() => setTExercises(p => p.map((e, j) => j === i ? { ...e, sets_target: Math.max(1, e.sets_target - 1) } : e))}
                      style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.08)', color: 'var(--text-secondary)', cursor: 'pointer', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.9rem' }}>-</button>
                    <span style={{ fontSize: '.8rem', minWidth: 40, textAlign: 'center', color: 'var(--text-secondary)' }}>{ex.sets_target}s</span>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                ...
                <button onClick={addExercise} style={{ padding: '0 14px', background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.10)', borderRadius: 11, color: 'var(--text-primary)', cursor: 'pointer', fontFamily: "'DM Sans','Tajawal',sans-serif", fontWeight: 700, fontSize: '.85rem' }}>Add</button>
              </div>
            </div>

            <button onClick={saveTemplate} disabled={!tName.trim() || saving}
              style={{ width: '100%', padding: '14px', background: tName.trim() ? '#111111' : 'rgba(0,0,0,0.08)', border: 'none', borderRadius: 9999, fontFamily: "'Space Grotesk','Tajawal',sans-serif", fontWeight: 800, fontSize: '.95rem', color: tName.trim() ? '#FFFFFF' : 'var(--text-secondary)', cursor: tName.trim() ? 'pointer' : 'not-allowed' }}>
