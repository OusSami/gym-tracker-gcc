...
### LOCKED STYLE — do not deviate

The canonical reference is `public/exercises/bilingual-curl-male.png` and `bilingual-curl-female.png`. Every exercise image must match these exactly:

- **Background**: pure white / very light grey studio. No gym, no equipment in background, no dark scenes.
- **Person**: photorealistic real human being. NEVER a 3D anatomical render, NEVER a cartoon or illustration.
- **Ghost**: the same person rendered faded/translucent, posed in the **START position** — always distinctly different from the main (end) pose. For bench press: ghost has bar at chest, elbows bent ~90°; main figure has arms extended. If the model duplicates the main pose for the ghost, generate a separate start-position render and composite it as a faded layer (≈30% opacity) behind the main figure using PIL. Never accept a ghost that mirrors the end pose.
- **Muscle highlight**: glowing overlay on skin/clothing — red for primary muscle(s), orange for secondary. Visible through clothing.
- **Framing**: full-body shot. Figure(s) centred, ample white space on both sides for labels.

### Modesty (non-negotiable)

| Gender | Clothing |
|--------|----------|
| Female | Hijab (head fully covered) + long-sleeve loose top + full-length loose pants + white sneakers. Zero skin except face and hands. |
| Male | Loose t-shirt (grey or dark) + full-length jogger pants + white sneakers. |

### Model & tool

```
node ~/tools/genimage.mjs "<prompt>" public/exercises/<slug>-<gender>-base.png
```

- Model: **gemini-2.5-flash-image** (set inside genimage.mjs). NEVER switch to gemini-2.0-pro-image or any other model — cost.
- Always save a `-base` (clean, no text) and a final (stamped) version separately.

### Label style (stamped by code — NEVER baked by the model)

Observe the curl reference images closely:

- Labels are **plain black text on white background** — no coloured pill boxes, no background rectangles.
- Each label = two lines: **`N  English Name`** on top (bold or semi-bold), **`الاسم العربي`** on the bottom in Arabic.
- Numbers (1, 2, 3 …) appear inline before the English name; the Arabic line is indented to align.
- **Leader lines**: thin black lines with a small filled arrowhead at the muscle end. Line runs from label text to the muscle on the figure.
- Labels are placed **outside** the figure — to the left for muscles on the left side, to the right for muscles on the right side.
- Arabic text must be RTL-correct: always run through `arabic_reshaper` + `bidi.algorithm.get_display` before rendering with Pillow. Font: `/System/Library/Fonts/SFArabic.ttf`. English font: `/System/Library/Fonts/SFNSMono.ttf` or system sans.
- Reference pose images are for **pose/form only** — never copy their art style or label style.

### Workflow per exercise (mandatory order)

**STEP 1 — label preview, wait for OK**

Before generating any full image:
1. Propose the EN + AR label list (muscle name, primary/secondary role).
2. Render a **small stamped preview image** (plain white canvas, labels + leader-line stubs) and show it visually. Do NOT show a terminal table — Arabic renders broken in the terminal.
3. **STOP. Wait for the user to confirm or correct labels before proceeding.**

**STEP 2 — generate bases, stamp, show, push**

Only after label OK:
1. Generate male base (no text): prompt must describe white studio background, real person, ghost figure, muscle highlights, modest clothing — match the curl images.
2. Generate female base (no text): same prompt adapted for hijab + modest clothing.
3. Save both bases as `<slug>-male-base.png` / `<slug>-female-base.png`.
4. Run the stamping script (Python + Pillow) to produce `<slug>-male.png` / `<slug>-female.png`.
5. **Show both stamped images to the user. Wait for approval before committing.**
6. On approval: `git add` the two final PNGs, commit, delete the `-base` working files.

### Cost & context hygiene

- Flash only — confirm the model string in `genimage.mjs` before each session.
- Run `/clear` between exercises to keep context lean.
- Lock the label list for an exercise before batch-generating; do not regenerate bases after labels are approved.
