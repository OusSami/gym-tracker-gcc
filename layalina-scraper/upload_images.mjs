...
/**
 * upload_images.mjs
 * Converts Layalina recipe images to WebP and uploads them to the SAME
 * "recipes" Supabase Storage bucket used by Sayidaty.
 *
 * Storage paths use "layalina-NNNN.webp" to avoid collisions with
 * Sayidaty's "recipe-NNNN.webp" naming.
 * (Supabase Storage rejects non-ASCII object keys, so we use padded index.)
 *
 * Writes layalina-scraper/data/image_url_map.json  { original_filename → public_url }
 *
...
const BUCKET     = 'recipes'   // shared with Sayidaty
...
if (!bucketExists) {
  ...
  console.log(`✓   Bucket "${BUCKET}" already exists (shared with Sayidaty)`)
}
...
for (let i = 0; i < allFiles.length; i++) {
  ...
  // "layalina-NNNN.webp" avoids collisions with Sayidaty's "recipe-NNNN.webp"
  const storagePath = `layalina-${String(i + 1).padStart(4, '0')}.webp`

  try {
    const origBuffer = fs.readFileSync(inputPath)
    totalOrigBytes  += origBuffer.length

    // Convert to WebP — same quality / effort as Sayidaty for consistency
    ...
    // Map original filename → public URL
    // (clean_recipes.mjs stores just the basename as image_local)
  ...
  if ((i + 1) % 10 === 0 || i + 1 === allFiles.length) {
    const origKB = (totalOrigBytes / 1024).toFixed(0)
    const webpKB = (totalWebpBytes / 1024).toFixed(0)
    console.log(`  ✓  ${i + 1}/${allFiles.length} — ${filename}  (orig ${origKB}KB → webp ${webpKB}KB so far)`)
  }
}
...
console.log(`   Storage bucket:        ${BUCKET}  (shared with Sayidaty)`)
console.log(`   Path prefix:           layalina-NNNN.webp`)
if (errors.length) {
